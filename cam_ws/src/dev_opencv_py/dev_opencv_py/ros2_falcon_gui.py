...
class ArucoSubscriber(Node):
    def __init__(self):
        ...
        # — GUI setup: resizable + fixed initial size —
        self.window_name = 'Aruco 3D Pose'
        cv2.namedWindow(self.window_name, cv2.WINDOW_NORMAL)       # enable resize :contentReference[oaicite:3]{index=3}
        cv2.resizeWindow(self.window_name, 1200, 800)             # set 1200×800 initial size :contentReference[oaicite:4]{index=4}
        cv2.setMouseCallback(self.window_name, self.on_mouse)

        # — Interaction state —
        self.dot_pos     = None    # (x, y) in original image coords
        self.rect_origin = None    # top-left of the marker‐based rectangle
        self.mpp_x       = None    # metres-per-pixel X
        self.mpp_y       = None    # metres-per-pixel Y
        self.last_tvec   = None    # [X, Y, Z] in metres from pose estimation
        self.frame_h     = 0       # original frame height for coordinate mapping

        # Store desired display size for scaling
        self.DISPLAY_W = 1200
        self.DISPLAY_H = 800

        self.get_logger().info(f"OpenCV {cv2.__version__} | Marker length = {self.marker_length} m")
    ...
    def image_callback(self, msg: Image):
            ...
            try:
                ...
                for c in corners:
                    ...
                    _, rv, tv = cv2.solvePnP(objp, imgp,
                                             self.camera_matrix, self.dist_coeffs)
                    rvecs_list.append(rv); tvecs_list.append(tv)
                rvecs = np.array(rvecs_list); tvecs = np.array(tvecs_list)

            # Compute rectangle & scaling
        ...
        display = cv2.resize(canvas,
                             ...
                             interpolation=cv2.INTER_LINEAR)  # efficient scaling :contentReference[oaicite:5]{index=5}
    ...
    def on_mouse(self, event, x, y, flags, param):
        ...
        # Map click back into original canvas coords
        fx = int(x * (self.frame_h / self.DISPLAY_H))
        fy = int(y * (self.frame_h / self.DISPLAY_H))
        ...
        # Place dot inside the 300×300 rect
