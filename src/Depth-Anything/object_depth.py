...
while cap.isOpened():
    ...
    # Iterate through contours and process each detected red object
    for cnt in contours:
        ...
        if area > 500:  # Filter out small contours; adjust threshold as needed
            ...
            if M["m00"] > 0:
                ...
                z = depth_normalized[int(center_y), int(center_x)]  # Depth value between 0-255
                # Optionally, map z to actual depth units if calibration is available
                ...
                text = f"X: {center_x}, Y: {center_y}, Z: {z}"
                cv2.putText(final_result, text, (center_x + 10, center_y + 10),
                            font, 0.5, (0, 255, 0), 1)

                # Print the coordinates to the terminal
                print(f"Detected Red Object - Center: ({center_x}, {center_y}), Depth (Z): {z:.2f}")

    # =======================

    # Write the annotated frame to the output video
    out_video.write(depth_color)  # You might want to write 'final_result' instead
