...
class DepthRedGrid6Node(Node):
    def __init__(self):
        ...
        self.X_fixed = 0.25  # Fixed X coordinate

        # Define predefined corner positions (for reference)
        # Not used in dynamic mapping
        self.grid_4 = {
            'top_left':      (0.25,  0.15, 0.15),
            'top_right':     (0.25, -0.15, 0.15),
            'bottom_left':   (0.25,  0.15, 0.00),
    ...
    def image_callback(self, msg):
        """ Main callback. Depth inference + Red detection + Mapping """
        try:
        ...
        # Resize for consistent usage (optional)
        ...
        # Fixed depth scaling with inversion
        # Assuming depth_resized is in meters. Adjust fixed_depth_min and fixed_depth_max as per your model's output.
        fixed_depth_min = 0.0  # meters
        fixed_depth_max = 10.0  # meters

        # Inverted scaling: closer objects have higher Z, far objects have lower Z
        depth_scaled = 3.0 - (depth_resized - fixed_depth_min) * (3.0 - 0.2) / (fixed_depth_max - fixed_depth_min + 1e-8)
        depth_scaled = torch.clamp(depth_scaled, 0.2, 3.0)  # Ensure values are within [0.2, 3.0]

        # For color map display: scale to [0,255]
        depth_uint8 = (depth_scaled / 3.0 * 255.0).cpu().numpy().astype(np.uint8)
        ...
        cv2.imshow("Depth Red Dynamic Mapping", final_result)
    ...
    def detect_red(self, hsv, final_result, width, height, caption_height):
        ...
        self.get_logger().info(f"Detected Red => center=({cx},{cy}), Position=(X: {X:.2f}, Y: {Y:.2f}, Z: {Z:.2f})")
    ...
    def map_center_to_coordinates(self, cx, cy, width, height):
        ...
        # Clamp the values to ensure they stay within [-0.15, 0.15]
