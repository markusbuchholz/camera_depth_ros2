...
class DepthRedGrid6Node(Node):
    def __init__(self):
        ...
        #for blue
        self.red_lower1 = np.array([80, 30, 80])
        self.red_upper1 = np.array([130, 255, 255])
        ...
        # -------------------------------------------------------------
        # 4. Depth scale (just for color map display)
        # -------------------------------------------------------------
    ...
    def detect_red(self, hsv, final_result, width, height, caption_height):
        ...
        self.get_logger().info(f"Detected Red => center=({cx},{cy}), picked {chosen_xyz}")
